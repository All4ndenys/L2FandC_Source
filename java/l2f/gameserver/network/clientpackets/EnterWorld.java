...
import l2f.gameserver.network.clientpackets.security.AbstractEnterWorldPacket;
	...
	protected void runImpl()
	{
		...
		if (client != null && Config.ALLOW_HWID_ENGINE)
		{
			...
			for (Player cha : GameObjectsStorage.getAllPlayersForIterate())
			{
				...
				instancesFound++;
				String charHwID = cha.getHWID();
				if (instancesFound >= Config.MAX_HWID_INSTANCES)
				{
					for (String hwids : Config.PRIVILEGED_HWID)
						if (!charHwID.contains(hwids))
						{
							Log.logClientProtection(activeChar.toString() + " bypassed max instance check trying to log more than the limit. Kicked from the game");
							cha.kick();
							return;
						}
				}
				if (instancesFound > Config.MAX_HWID_INSTANCES_PRIVILEGED)
				{
					Log.logClientProtection("Privileged HwID" +activeChar.toString() + " bypassed SGuard max instance check trying to log more than the limit. Kicked from the game");
					cha.kick();
					return;
				}
			}
		...
		if (first)
		{
			...
			// We also give player spawn protection, so he cannot receive damage until he does something
		...
		// SevenDays Reward
		if(SevenDaysReward.showWindow(activeChar, activeChar.getIP(), client.getHWID()))
		{
			SevenDaysReward.reward(activeChar);
			
			NpcHtmlMessage html = new NpcHtmlMessage(0).setFile("DailyReward.htm");
			html.replace("%name%", activeChar.getName());
