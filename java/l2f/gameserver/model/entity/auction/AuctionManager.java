...
public class AuctionManager
{
	...
	private void loadAuctions()
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("SELECT * FROM auctions"); ResultSet rset = statement.executeQuery())
		{
			while (rset.next())
			{
				...
				int currency = rset.getInt("currency");
				...
				if (item != null)
				{
					Auction auction = new Auction(id, sellerObjectId, sellerName, item, pricePerItem, item.getCount(), getItemGroup(item), false, currency);
	...
	public void addAuctionToDatabase(Auction auction)
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("INSERT INTO auctions VALUES(?,?,?,?,?,?)"))
		{
			...
			statement.setInt(6,auction.getCurrency());
			statement.execute();
		}
		catch (SQLException e)
		{
    ...
    public void addCoinsToSeller(int id, int sellerObjectId, long amount) {
        int objId = -1;

        try (Connection con = DatabaseFactory.getInstance().getConnection();
             PreparedStatement statement = con.prepareStatement("SELECT object_id FROM items WHERE item_id=" + id + " AND owner_id=" + sellerObjectId + " AND loc='INVENTORY'");
        ...
        if (objId == -1) {
            ...
            try (Connection con = DatabaseFactory.getInstance().getConnection();
                 PreparedStatement statement = con.prepareStatement("UPDATE items SET count=count+" + amount + " WHERE object_id=" + objId)) {
                statement.execute();
            } catch (SQLException e) {
                _log.error("Error while selecting adena:", e);
            }
	...
	public Auction addNewStore(Player seller, ItemInstance item, long salePrice, long count)
	{
		...
		return addAuction(seller, id, item, salePrice, count, type, true, 57);
	}
	...
	public void buyItem(Player buyer, ItemInstance item, long quantity)
	{
		...
		if (auction.getPricePerItem() * quantity > buyer.getItemCountByID(auction.getCurrency()))
		{
			sendMessage(buyer, "You don't have enough " + auction.getCurrencyName(auction.getCurrency()));
			return;
		}
		...
		final long totalCost = auction.getPricePerItem() * quantity;
		logs.add(new ItemActionLog(ItemStateLog.EXCHANGE_LOSE, "AuctionHouse", buyer, buyer.getInventory().getItemByItemId(auction.getCurrency()), totalCost));
		buyer.getInventory().reduceAuctionCurrency(auction.getCurrency(), auction.getPricePerItem() * quantity, "Auction Online Bought");
		...
		if (seller2 != null)
		{
			...
			seller2.addAuctionCurrency(auction.getCurrency(), auction.getPricePerItem() * quantity, true, null);
			logs.add(new ItemActionLog(ItemStateLog.EXCHANGE_GAIN, "AuctionHouse", seller2, seller2.getInventory().getItemByItemId(auction.getCurrency()), auction.getPricePerItem() * quantity));
		}
		else
		{
			//addAdenaToSeller(auction.getSellerObjectId(), auction.getPricePerItem() * quantity);
			addCoinsToSeller(auction.getCurrency(), auction.getSellerObjectId(), auction.getPricePerItem() * quantity);
			logs.add(new ItemActionLog(ItemStateLog.EXCHANGE_GAIN, "AuctionHouse", auction.getSellerName() + '[' + auction.getSellerObjectId() + ']', "Adena", totalCost));
		}
	...
	public void checkAndAddNewAuction(Player seller, ItemInstance item, long quantity, long salePrice, int currency)
	{
		if (!checkIfItsOk(seller, item, quantity, salePrice, currency))
			return;
		...
		try
		{
			if (item.getCount() > quantity)
			{
				...
				auction = addAuction(seller, id, newItem, salePrice, quantity, type, false, currency);
			}
			else
			{
				...
				auction = addAuction(seller, id, item, salePrice, quantity, type, false, currency);
			}
		}
		finally
		{
	...
	private Auction addAuction(Player seller, int auctionId, ItemInstance item, long salePrice, long sellCount, AuctionItemTypes itemType, boolean privateStore, int currency)
	{
		Auction newAuction = new Auction(auctionId, seller.getObjectId(), seller.getName(), item, salePrice, sellCount, itemType, privateStore, currency);
	...
	private boolean checkIfItsOk(Player seller, ItemInstance item, long quantity, long salePrice, int currency)
	{
		if (seller == null)
			return false;
        ...
        if (item.getItemId() == currency) {
            sendMessage(seller, "You cannot sell items, of the same kind. Use different coin auction in order to sell this.");
            return false;
        }
