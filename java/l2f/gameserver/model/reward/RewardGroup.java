	...
	private void rollFinal(List<RewardData> items, List<RewardItem> ret, double mult, double chanceSum)
	{
		...
		for (RewardData i : items)
		{
			if (chance < i.getChanceInGroup() && chance > i.getChanceInGroup() - i.getChance())
			{
				...
				/*// claw - no drop vesper vorpal elegia.
				if(i.getItem().getName().contains("Vesper") || i.getItem().getName().contains("Vorpal") || i.getItem().getName().contains("Elegia"))
				{
					count = 0;
					break;
				}*/
				...
				String _itemName = i.getItem().getName();
				for (String item : Config.NO_DROP_ITEMS)
				{
					//debug System.out.println("OUT-itemName: " + _itemName + " / item : " + item);
					if (_itemName.contains(item))
					{
						//debug System.out.println("IN-itemName: " + _itemName + " / item : " + item);
						count = 0;
					}
				}
				
				//debug System.out.println("itemName: " + _itemName);
				
				String _itemId = String.valueOf(i.getItem().getItemId());
				for (String item : Config.NO_DROP_ITEMS)
				{
				//debug System.out.println("OUT-itemId: " + _itemId + "/ item: " + item);
					if (_itemId.equals(item))
					{
					//debug	System.out.println("IN-itemId: " + _itemId + "/ item: " + item);
						count = 0;
					}
				}
				//debug System.out.println("_itemId: " + _itemId);

				//debug System.out.println("LAST_ItemName: " + _itemName + " / count: " + count);
				RewardItem t = null;
