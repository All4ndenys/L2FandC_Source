...
public class AuthRequest extends ReceivablePacket
{
  ...
  //  private ArrayList<AdvIP> advIpList;
	...
	protected void readImpl()
	{
		...
		final int advIpsSize = readD();
		advIpList = new ArrayList<>();
		for (int i = 0; i < advIpsSize; i++)
		{
		        final AdvIP ip = new AdvIP();
		        ip.channelId = readD();
	...
	protected void runImpl()
	{
		...
		//	gs.setAdvIP(advIpList);
			...
			for (final AdvIP localAdvIP : gs.getAdvIP())
				GameServerManager.getInstance().registerGameServerChannel(localAdvIP.channelId, gs);*/
		}
		else if (acceptAlternateID)
		{
			int latinId = 2;
			if (GameServerManager.getInstance().registerGameServer(latinId,gs = getGameServer()))
			{
				gs.setPorts(ports);
				gs.setExternalHost(externalIp);
				gs.setInternalHost(internalIp);
