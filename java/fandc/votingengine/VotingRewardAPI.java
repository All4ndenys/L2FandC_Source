...
public class VotingRewardAPI implements IVoicedCommandHandler, Runnable
{
	...
	protected VotingRewardAPI()
	{
		...
		_log.info("VoteReward: Loaded Successfully!");
	...
	private void reloadApi()
	{
		...
		VotingRewardCache.getInstance().load();
		_task = ThreadPoolManager.getInstance().scheduleAtFixedRate(this, 10000, 5000);
		_log.info("VoteReward: Reloaded successfully");
	}
	...
	public boolean useVoicedCommand(String command, Player player, String params)
	{
		...
		{
			player.sendMessage("You request is already in queue, please wait..");
			return false;
		}
		player.sendMessage("Your reward request has been added to the queue...");
		return true;
	}
