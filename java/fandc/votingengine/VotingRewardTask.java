...
public class VotingRewardTask
{
	...
	public static void checkReward(Player player)
	{
		...
		//System.out.println("timeRemaning: " + timeRemaining);
		if (timeRemaining <= 0)
		{
			for (String zone : VotingSettings.getInstance().getZones().keySet())
			{
				//System.out.println("getIP isVotter: " + isVotter(zone, player.getIP()));
				//System.out.println("getNe isVotter: " + isVotter(zone, player.getNetConnection().getIpAddr()));
	...
	private static final boolean isVotter(String zoneName, String ip)
	{
		try
		{
			...
			if (responseCode == 200) // OK
			{
				...
				if (zoneName.equals("Topzone")){
					//System.out.println("Parse Topzone: " + parseTopzone(sb));
					return parseTopzone(sb);}
				else if (zoneName.equals("Hopzone"))
					//System.out.println("Parse Hopzone: " +parseHopzone(sb));
